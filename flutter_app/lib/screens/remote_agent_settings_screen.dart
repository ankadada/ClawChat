...
class _RemoteAgentSettingsScreenState extends State<RemoteAgentSettingsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        _baseUrl.text = config?.baseUrl ?? _baseUrl.text;
        _agentId.text = config?.remoteAgentId ?? _agentId.text;
  ...
  Future<void> _removeCredential() async {
    ...
    try {
      ...
      _credential.clear();
      _disclosureAccepted = false;
      if (mounted) setState(() => _hasCredential = false);
      _show('凭据和远程配置已移除。');
    } on Object {
          ...
          : Form(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  DropdownButtonFormField<RemoteAgentConnectorKind>(
                    value: RemoteAgentConnectorKind.cozeOpenApi,
                  ...
                  TextFormField(
                    controller: _agentId,
                    decoration: const InputDecoration(
                      labelText: '远程 Agent ID',
                      border: OutlineInputBorder(),
                    ),
                    validator: (value) =>
                        (value ?? '').trim().isEmpty ? '请输入远程 Agent ID' : null,
                  ),
                  const SizedBox(height: 12),
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      labelText: _hasCredential ? '替换凭据（留空则保留）' : '凭据',
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const Text(
                            '启用后，仅当你在某个对话中明确选择远程 Agent 时，当前本地对话文本会发送到所配置的外部服务处理。ClawChat 不同步云端会话，不持久化服务商会话 ID、原始流帧或请求元数据。',
                          ),
                          CheckboxListTile(
