...
function printBanner() {
  const title = `OpenClaw-Termux v${VERSION}`;
  const subtitle = 'AI Gateway for Android';
  const boxWidth = 43; // inner width between ║ characters
  const padTitle = ('     ' + title).padEnd(boxWidth);
  const padSubtitle = ('     ' + subtitle).padEnd(boxWidth);
  console.log(`
╔${'═'.repeat(boxWidth)}╗
║${padTitle}║
║${padSubtitle}║
...
async function runSetup() {
  ...
  let status = getInstallStatus(true);
  ...
  if (!status.proot) {
    ...
    if (!installProot()) {
      console.error('Setup failed at step 1: proot-distro installation.');
      process.exit(1);
    }
  } else {
  ...
  status = getInstallStatus(true);
  if (!status.ubuntu) {
    console.log('  Installing Ubuntu (this takes a while)...');
    if (!installUbuntu()) {
      console.error('Setup failed at step 2: Ubuntu installation.');
      process.exit(1);
    }
  } else {
  ...
  if (!status.openClawInProot) {
    if (!setupProotUbuntu()) {
      console.error('Setup failed at step 3: Node.js/OpenClaw installation in Ubuntu.');
      process.exit(1);
    }
  } else {
  ...
  if (!setupBionicBypassInProot()) {
    console.error('Setup failed at step 4: Bionic Bypass configuration.');
    process.exit(1);
  }
  console.log('');
  ...
  if (!configureTermux()) {
    console.error('Setup failed at step 5: Termux configuration.');
    process.exit(1);
  }
  console.log('');
...
function runOpenclawCommand(args) {
  ...
  const proc = runInProot('openclaw', args);
