...
async function runTests() {
  ...
  test('bypass handles networkInterfaces crash', () => {
    const originalFn = os.networkInterfaces;

    // Simulate Bionic libc crash
    os.networkInterfaces = () => { throw new Error('Bionic libc crash'); };

    // Re-apply bypass: wraps the (now-broken) function with fallback
    const bypassFn = () => {
      try {
        const interfaces = originalFn.call(os);
    ...
    os.networkInterfaces = bypassFn;
    const result = os.networkInterfaces();
    assert.ok(result.lo, 'Should return fallback lo interface');
    assert.equal(result.lo[0].address, '127.0.0.1', 'Should have localhost address');

    // Restore original
    os.networkInterfaces = originalFn;
  });
